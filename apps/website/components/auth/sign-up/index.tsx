...
import { signUpApi, setStoredTokens } from '@/lib/auth-api'
...
const SignUp = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.success('Check your email or sign in.')
              ...
              <div className='relative shadow-lg mx-auto max-w-32 overflow-hidden rounded-lg bg-white dark:bg-dark_black px-8 py-14 text-center dark:bg-dark-2 sm:px-12 md:px-16'>
                ...
                <form onSubmit={handleSubmit}>
                  <div className='mb-5 text-left'>
                    ...
                    />
                    {errors.name && (
                      <p className='text-red-500 dark:text-red-500 text-sm mt-1'>{errors.name}</p>
                    )}
                  </div>
                  <div className='mb-5 text-left'>
                    <input
                    ...
                    />
                    {errors.email && (
                      <p className='text-red-500 dark:text-red-500 text-sm mt-1'>{errors.email}</p>
                    )}
                  </div>
                  <div className='mb-5 text-left'>
                    <input
                    ...
                    />
                    {errors.password && (
                      <p className='text-red-500 dark:text-red-500 text-sm mt-1'>{errors.password}</p>
                    )}
                  </div>
                  <div className='mb-9'>
                    <button
                ...
                <div className='flex flex-col max-w-xs mx-auto gap-2'>
                  <p className='text-dark_black/70 dark:text-white/50'>
                    ...
                    <Link href='/privacy-policy' className='text-dark_black dark:text-white dark:hover:text-white-50'>
                      Privacy
                    </Link>{' '}
                    and{' '}
                    <Link href='/privacy-policy' className='text-dark_black dark:text-white dark:hover:text-white-50'>
                  ...
                  <p className='text-dark_black/70 dark:text-white/50'>
                    ...
                    <Link href='/signin' className='text-dark_black dark:text-white dark:hover:text-white/50'>
                      Sign In
                    </Link>
                  </p>
                </div>
